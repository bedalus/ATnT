...
//#define DHD_RXBOUND	128	/* Default for max rx frames in one scheduling */

//#define DHD_TXBOUND	20	/* Default for max tx frames in one scheduling */
#define DHD_TXBOUND	256	/* Default for max tx frames in one scheduling */
...
dhd_bus_txctl(struct dhd_bus *bus, uchar *msg, uint msglen)
{
	...
	if (bus->dhd->dongle_reset) {
		DHD_ERROR(("[DEBUG] %s: dongle_reset is true\n", __FUNCTION__));
		return -EIO;
	}
...
dhdsdio_download_state(dhd_bus_t *bus, bool enter)
{
	...
	if (enter) {
		...
		if (REMAP_ENAB(bus) && si_socdevram_remap_isenb(bus->sih)) {
			DHD_ERROR(("%s: Disable remap for download\n", __FUNCTION__));
		...
		if (bus->ramsize) {
			...
			if (dhdsdio_membytes(bus, TRUE, bus->ramsize - 4, (uint8*)&zeros, 4) < 0) {
				DHD_ERROR(("%s: dhdsdio_membytes Failed\n", __FUNCTION__));
				bcmerror = BCME_SDIO_ERROR;
				goto fail;
			}
		}
			...
			*/
			...
			DHD_ERROR(("%s: Halt ARM & remove reset\n", __FUNCTION__));
